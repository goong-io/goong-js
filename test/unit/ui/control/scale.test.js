...
test('ScaleControl appears in bottom-left by default', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.goongjs-ctrl-bottom-left .goongjs-ctrl-scale').length, 1);
    t.end();
});
...
test('ScaleControl appears in the position specified by the position option', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.goongjs-ctrl-top-left .goongjs-ctrl-scale').length, 1);
    t.end();
});
...
test('ScaleControl should change unit of distance after calling setUnit', (t) => {
    ...
    const selector = '.goongjs-ctrl-bottom-left .goongjs-ctrl-scale';
